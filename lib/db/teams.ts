...
export async function leaveTeam(teamId: string, userId: string) {
    ...
    if (!normalizedTeamId || !normalizedUserId) return;

    if (isSupabaseBackend()) {
        const rows = (await supabaseRest(
            `/team_members?select=id,status&team_id=eq.${encodeURIComponent(normalizedTeamId)}&user_id=eq.${encodeURIComponent(normalizedUserId)}&limit=1`
        )) as Array<{ id: string; status: string }>;
        if (!rows.length) return;

        await supabaseRest(
            `/team_members?id=eq.${encodeURIComponent(rows[0].id)}`,
            {
                method: "PATCH",
                prefer: "return=minimal",
    ...
    const page = (existing.results as any[])[0];
    if (!page?.id) return;

    const currentStatusProperty = page.properties?.status;
    const statusPayload =
        currentStatusProperty?.type === "status"
