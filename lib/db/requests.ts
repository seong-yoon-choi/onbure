...
function mapChatRequestAsFriend(item: RequestItem): RequestItem {
    return {
        ...item,
        type: "FRIEND",
    };
...
export async function createFriendRequest(fromUserId: string, toUserId: string, message: string) {
    try {
        await createChatRequest(fromUserId, toUserId, message);
    } catch (error) {
        if (error instanceof RequestConflictError) {
            throw new RequestConflictError(error.message, "FRIEND");
        }
        throw error;
    }
}

export async function getFriendRequestsForUserByStatuses(
    userId: string,
    statuses: RequestStatus[]
): Promise<RequestItem[]> {
    const items = await getChatRequestsForUserByStatuses(userId, statuses);
    return items.map(mapChatRequestAsFriend);
}

export async function getAcceptedConnectionPartnerIds(userId: string): Promise<string[]> {
    return getAcceptedChatPartnerIds(userId);
}
...
export async function getActiveFriendPartnerStates(
    ...
    return getActiveChatPartnerStates(userId);
}

export async function removeFriendship(userId: string, friendUserId: string): Promise<void> {
    const left = String(userId || "").trim();
    const right = String(friendUserId || "").trim();
    if (!left || !right || left === right) return;

    if (isSupabaseBackend()) {
        await Promise.all([
            supabaseRest(
                `/chat_requests?from_user_id=eq.${encodeURIComponent(left)}&to_user_id=eq.${encodeURIComponent(right)}`,
                { method: "DELETE", prefer: "return=minimal" }
            ),
            supabaseRest(
                `/chat_requests?from_user_id=eq.${encodeURIComponent(right)}&to_user_id=eq.${encodeURIComponent(left)}`,
                { method: "DELETE", prefer: "return=minimal" }
            ),
        ]);
        return;
    ...
    const existing = await notion.databases.query({
        ...
        filter: {
            and: [
                {
                    or: [
                        { property: "from_user_id", rich_text: { equals: left } },
                        { property: "to_user_id", rich_text: { equals: left } },
                    ],
                },
                {
                    or: [
                        { property: "from_user_id", rich_text: { equals: right } },
                        { property: "to_user_id", rich_text: { equals: right } },
                    ],
                },
            ],
        },
    ...
    await Promise.all(
        (existing.results as any[]).map((page) =>
            notion.pages.update({
                page_id: page.id,
                archived: true,
            })
